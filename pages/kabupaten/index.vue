        ...
        <b-card class="shadow-md border-0 rounded-lg">
          ...
          >
            <template v-slot:cell(actions)="row">
              <b-button
                :to="{ name: 'kabupaten-edit-id', params: { id: row.item.id } }"
                variant="warning"
                size="sm"
                >EDIT</b-button
              >
              <b-button variant="danger" size="sm" @click="deleteKabupaten(row)"
                >DELETE</b-button
              >
            </template>
          </b-table>
...
export default {
  ...
  methods: {
    async deleteKabupaten(row) {
      ...
      await this.$axios.delete(`/api/kabupaten/${row.item.id}`).then(() => {
