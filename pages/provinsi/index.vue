        ...
        <b-card class="shadow-md border-0 rounded-lg">
          ...
          >
            <template v-slot:cell(actions)="row">
              <b-button
                :to="{ name: 'provinsi-edit-id', params: { id: row.item.id } }"
                variant="warning"
                size="sm"
                >EDIT</b-button
              >
              <b-button variant="danger" size="sm" @click="deleteProvinsi(row)"
                >DELETE</b-button
              >
            </template>
          </b-table>
...
export default {
  ...
  methods: {
    async deleteProvinsi(row) {
      ...
      await this.$axios.delete(`/api/provinsi/${row.item.id}`).then(() => {
